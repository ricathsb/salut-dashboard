...
export async function GET(request: Request) {
  try {
    ...
    const registration = dummyRegistrations.find((reg) => reg.id === id)

    if (!registration) {
      return NextResponse.json({ error: "Registration not found" }, { status: 404 })
    }

    console.log("Download document for registration ID:", id)

    // Simulasi pembuatan file CSV untuk data spesifik
    const csvHeaders = [
      "ID",
      "Nama Lengkap",
      "NIK",
    ...
    return new NextResponse(csvContent, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="registrasi-${registration.namaLengkap.replace(/\s+/g, "-")}.csv"`,
      },
    })
  } catch (error) {
